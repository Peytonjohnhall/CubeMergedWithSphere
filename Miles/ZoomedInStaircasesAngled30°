...
def plot_staircases(ax, color='tan'):
    """
    Plots the staircases angled at 30°.
    ...
    sqrt_3_over_3 = np.sqrt(3) / 3
    x1 = np.linspace(-848.995, -689.4886364, 100)
    y1 = (sqrt_3_over_3 * x1 + 689.4886364) - 291.412

    x2 = np.linspace(689.4886364, 848.995, 100)
    y2 = -((sqrt_3_over_3 * x2 - 689.4886364) + 291.412)
